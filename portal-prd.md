...
- **Lucy returns ~18fps, not 30** (§2.3.1), on both codecs, while we publish a full 30. **Confirmed as the model's generation rate, not transport: the same connection carries 29fps in passthrough.** Our own render loop was cleared separately at a flat 60fps. Choppiness reads as latency, so it may be costing more perceptually than the raw Δ. **Open with Decart, alongside the inference time.** Also untested: whether `lucy-restyle-2` behaves differently — note it is 1280×704, so trying it costs an aspect-ratio change (§3).
